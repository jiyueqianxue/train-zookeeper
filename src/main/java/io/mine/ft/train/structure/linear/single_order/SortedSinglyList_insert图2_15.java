package io.mine.ft.train.structure.linear.single_order;
...
public class SortedSinglyList_insert图2_15<T extends Comparable<? super T>> extends SinglyList<T>
{    
    public SortedSinglyList_insert图2_15()                              //构造空排序单链表
    {
        super();                                           //默认调用父类构造方法SinglyList()
    }
    
    public SortedSinglyList_insert图2_15(T[] values)                    //构造，按值插入values数组元素。直接插入排序算法
    {
    ...
    public SortedSinglyList_insert图2_15(SortedSinglyList_insert图2_15<T> list)      //深拷贝构造方法
    {
        super(list);                                       //调用父类同参数的构造方法，不可省略
    }   

    //由单链表list构造（深拷贝），重载构造方法。直接插入排序
    public SortedSinglyList_insert图2_15(SinglyList<T> list)
    {
        super();                                           //构造空单链表
...
//        System.out.println();
    ...
    public SortedSinglyList_insert图2_15<T> union(SinglyList<T> list)
    {
        SortedSinglyList_insert图2_15<T> result = new SortedSinglyList_insert图2_15<T>(this);   //排序单链表深拷贝
        result.addAll(list);
        return result;
    }
...
//            System.out.println(this.toString());
    ...
    //9.5.2算法描述、程序
    public void merge(SortedSinglyList_insert图2_15<T> list)            //将list所有结点归并到this中，归并后设置list为空
    {
...
/*    public SortedSinglyList<T> mergeWith(SortedSinglyList<T> list)  
    ...
    public SortedSinglyList_insert图2_15<T> mergeWith(SortedSinglyList_insert图2_15<T> list)  
    {
        Node<T> p=this.head.next, q=list.head.next;        //p、q分别遍历this、list单链表
        SortedSinglyList_insert图2_15<T> result = new SortedSinglyList_insert图2_15<T>();
